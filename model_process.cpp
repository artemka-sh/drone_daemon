...
Result ModelProcess::LoadModelFromFileWithMem(const char *modelPath) {
    ...
    if (ret != ACL_ERROR_NONE) {
        ERROR_LOG("malloc buffer for mem failed");
        return FAILED;
    }

    ret = aclrtMalloc(&modelWeightPtr_, modelWeightSize_, ACL_MEM_MALLOC_HUGE_FIRST);
    if (ret != ACL_ERROR_NONE) {
        ERROR_LOG("malloc buffer for weight failed");
        return FAILED;
    }
...
Result ModelProcess::CreateDesc()
{
    ...
    if (modelDesc_ == nullptr) return FAILED;
...
Result ModelProcess::CreateInput(void *input1, size_t input1size, void* input2, size_t input2size)
{
    ...
    if (input_ == nullptr) return FAILED;

    aclDataBuffer* inputData = aclCreateDataBuffer(input1, input1size);
    if (inputData == nullptr) return FAILED;
    ...
    if (inputData2 == nullptr) return FAILED;
...
Result ModelProcess::CreateOutput()
{
    ...
    if (output_ == nullptr) return FAILED;
